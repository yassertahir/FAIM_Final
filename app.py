    ...
    try:
        ...
        with open(STORAGE_FILE, "w") as f:
            json.dump({
                ...
                "thread_id": thread_id  # Save thread_id
            }, f)
...
with st.sidebar:
    ...
    # Store file metadata for maintaining context
        ...
        if st.button("Analyze All Files"):
            ...
            if file_ids:
                ...
                with st.spinner("Analyzing files..."):
                    ...
                    Your goal is to extract as many valuation inputs as possible from the content. If any key inputs are missing or ambiguous, prompt the user clearly to provide or confirm them.
                    ...
                    Use the following structure:
                        ...
                        Multiply: weight × multiplier × median valuation.
...
if user_input:
    ...
    has_processed_files = len(st.session_state.get("processed_files", set())) > 0
    
    # Add context reminder about uploaded documents
    if has_processed_files:
        final_user_input = f"{user_input}\n\n(Remember to reference the previously uploaded documents in your response. Follow the VC evaluation framework and refuse irrelevant requests.)"
    else:
        final_user_input = f"{user_input}\n\n(Please follow the VC evaluation framework and refuse irrelevant requests.)"
